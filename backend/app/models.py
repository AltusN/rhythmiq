...
from sqlalchemy import Boolean, CheckConstraint, Date, Enum, ForeignKey, Integer, String, UniqueConstraint
...
class Club(Base):
    ...
    gymnasts: Mapped[list["Gymnast"]] = relationship("Gymnast", back_populates="club", passive_deletes=True)
    coaches: Mapped[list["Coach"]] = relationship("Coach", back_populates="club", passive_deletes=True)
...
class MeetEntry(Base):
    ...
    routines: Mapped[list["Routine"]] = relationship("Routine", back_populates="entry", cascade="all, delete-orphan")
