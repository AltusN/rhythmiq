...
class TestJudgeCreate:
    ...
    def test_create_judge_missing_first_name(self):
        data = {"last_name": "Doe", "country_code": "USA", "brevet": "A"}
        with pytest.raises(ValidationError):
            JudgeCreate.model_validate(data)

    def test_create_judge_missing_last_name(self):
        data = {"first_name": "John", "country_code": "USA", "brevet": "A"}
        with pytest.raises(ValidationError):
            JudgeCreate.model_validate(data)

    def test_create_judge_country_code_and_brevet_omitted(self):
        # country_code/brevet aren't in the payload at all (not even as None) --
        # distinct from test_create_judge_country_code_none, which sends the key explicitly.
        data = {"first_name": "John", "last_name": "Doe"}
        judge = JudgeCreate.model_validate(data)
        assert judge.country_code is None
        assert judge.brevet is None
...
class TestJudgeUpdate:
    def test_update_judge_valid(self):
        data = {"first_name": "Jane", "last_name": "Smith", "country_code": "CAN", "brevet": "B"}
    ...
    def test_update_judge_brevet_only(self):
        data = {"brevet": "C"}
        judge = JudgeUpdate.model_validate(data)
        assert judge.first_name is None
        assert judge.last_name is None
        assert judge.country_code is None
        assert judge.brevet == "C"
    ...
    def test_update_judge_exclude_unset_only_includes_provided_fields(self):
        ...
        judge = JudgeUpdate.model_validate({"brevet": "C"})
        assert judge.model_dump(exclude_unset=True) == {"brevet": "C"}
...
class TestJudgeRead:
    ...
    def test_read_judge_from_orm_like_object(self):
        class JudgeORM:
            def __init__(self, id, first_name, last_name, country_code, brevet):
                self.id = id
        ...
        orm_judge = JudgeORM(3, "Alice", "Johnson", "CAN", "B")
