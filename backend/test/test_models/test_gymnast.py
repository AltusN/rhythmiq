...
def test_gymnast_create_with_required_fields(db_session):
    ...
    assert (
        db_session.query(Gymnast).filter_by(first_name="Ivanka", last_name="Kuznetsova").first()
        is not None
    )
...
def test_gymnast_create_with_optional_fields_null(db_session):
    gymnast = Gymnast(first_name="Liam", last_name="Smith", date_of_birth=None, country_code=None)
    db_session.add(gymnast)
    db_session.commit()

    assert (
        db_session.query(Gymnast).filter_by(first_name="Liam", last_name="Smith").first()
        is not None
    )
...
def test_gymnast_first_name_too_short_raises_error(db_session):
    gymnast = Gymnast(
        first_name="A", last_name="Smith", date_of_birth=date(2010, 1, 1), country_code="USA"
    )
    db_session.add(gymnast)
...
def test_gymnast_first_name_required(db_session):
    gymnast = Gymnast(
        first_name=None, last_name="Doe", date_of_birth=date(2010, 1, 1), country_code="USA"
    )
    db_session.add(gymnast)
    with pytest.raises(IntegrityError):
        db_session.commit()
...
def test_gymnast_last_name_required(db_session):
    gymnast = Gymnast(
        first_name="John", last_name=None, date_of_birth=date(2010, 1, 1), country_code="USA"
    )
    db_session.add(gymnast)
    with pytest.raises(IntegrityError):
        db_session.commit()
...
def test_duplicate_gymnast_names_with_different_dob_allowed(db_session):
    gymnast1 = Gymnast(
        first_name="Alex", last_name="Johnson", date_of_birth=date(2010, 1, 1), country_code="USA"
    )
    db_session.add(gymnast1)
    db_session.commit()

    gymnast2 = Gymnast(
        first_name="Alex", last_name="Johnson", date_of_birth=date(2011, 2, 2), country_code="USA"
    )
...
def test_gymnast_unique_identity_constraint(db_session):
    gymnast1 = Gymnast(
        first_name="Unique", last_name="Gymnast", date_of_birth=date(2010, 1, 1), country_code="USA"
    )
    db_session.add(gymnast1)
    db_session.commit()

    gymnast2 = Gymnast(
        first_name="Unique", last_name="Gymnast", date_of_birth=date(2010, 1, 1), country_code="CAN"
    )
    db_session.add(gymnast2)
    with pytest.raises(IntegrityError):
        db_session.commit()
...
def test_gymnast_cascasde_delete(db_session):
    ...
    assert (
        db_session.query(Meet).filter_by(id=meet.id).first() is not None
    )  # Meet should still exist
...
def test_gymnast_delete_with_multiple_meet_entries(db_session):
    ...
    assert (
        db_session.query(Meet).filter_by(id=meet1.id).first() is not None
    )  # Meets should still exist
