...
def test_coach_create_with_optional_fields_null(db_session):
    ...
    assert (
        db_session.query(Coach).filter_by(first_name="Jane", last_name="Smith").first() is not None
    )
    assert db_session.query(Coach).filter_by(club_id=club.id).first() is not None
...
def test_multiple_coaches_same_club(db_session):
    ...
    assert (
        db_session.query(Coach).filter_by(first_name="Alice", last_name="Brown").first() is not None
    )
    assert (
        db_session.query(Coach).filter_by(first_name="Bob", last_name="Green").first() is not None
    )
...
def test_multiple_clubs_with_head_coaches(db_session):
    ...
    assert (
        db_session.query(Coach).filter_by(first_name="Carol", last_name="White").first() is not None
    )
    assert (
        db_session.query(Coach).filter_by(first_name="Dave", last_name="Black").first() is not None
    )
    assert (
        db_session.query(Coach).filter_by(club_id=club1.id, is_head_coach=True).first() is not None
    )
    assert (
        db_session.query(Coach).filter_by(club_id=club2.id, is_head_coach=True).first() is not None
    )
...
def test_coach_can_become_head_coach(db_session):
    ...
    assert (
        ...
        .filter_by(first_name="Eve", last_name="Davis", is_head_coach=True)
        .first()
        is not None
    )
...
def test_coach_relationship_with_club(db_session):
    ...
    fetched_coach = (
        db_session.query(Coach).filter_by(first_name="Frank", last_name="Miller").first()
    )
